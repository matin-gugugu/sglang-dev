...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/experiment-results/phase12/qwen3_30b_a3b_pattern_demand}"
...
assert all((model / name).is_file() and (model / name).stat().st_size for name in shards)
...
check_admission() {
  ...
  if [[ "$tp" == 4 ]]; then
    [[ -s "$REPO_ROOT/experiment-results/phase12/qwen3_30b_a3b_admission/tp2/DONE" \
      ...
      printf 'TP=4 requires successful Qwen3-30B-A3B TP=2 and TP=8 admission\n' >&2
      return 2
    }
  else
    [[ -s "$REPO_ROOT/experiment-results/phase12/qwen3_30b_a3b_admission/tp${tp}/DONE" ]] || {
      printf 'missing successful Qwen3-30B-A3B admission TP=%s\n' "$tp" >&2
      return 2
    }
  fi
}
...
assert all(row["generated_output_tokens"] == row["output_len"] for row in rows)
assert all(
    len(row["generated_output_tokens_per_request"]) == row["batch_size"]
    and set(row["generated_output_tokens_per_request"]) == {row["output_len"]}
    for row in rows
)
keys = {(row["batch_size"], row["input_len"], row["output_len"]) for row in rows}
...
for row in rows:
    ...
    phase_histograms = [
        item for item in representative["event_histograms"] if item["phase"] == phase
    ]
    ...
    call_counts.add(sum(item["count"] for item in phase_histograms))

print(
    f"validated {directory}: rows={len(rows)} phase={phase} tp={tp} "
    f"payload_sizes={len(payload_sizes)} calls={sorted(call_counts)} ops={sorted(ops)}"
)
PY
}
...
run_phase_grid() {
  ...
  check_gpus_idle "$tp"
  mkdir -p "$directory"
  rm -f "$directory/result.jsonl" "$directory/run.log" \
    "$directory/telemetry.csv" "$directory/validate.log" "$done_marker"

  printf 'start Qwen3-30B-A3B PatternDemand TP=%s repeat=%s phase=%s expected=%s\n' \
  ...
  CUDA_VISIBLE_DEVICES="$(visible_devices "$tp")" PYTHONPATH=python \
    python -m sglang.benchmark.one_batch \
      ...
      --run-name "qwen3-30b-a3b-pattern-tp${tp}-${phase}-r${repeat}" \
  ...
  if grep -Eqi 'out of memory|Traceback|CPU fallback|falling back' "$directory/run.log"; then
    printf 'run log contains an error or fallback marker: %s\n' \
      "$directory/run.log" >&2
    return 1
  fi
  printf 'complete\n' >"$done_marker"
