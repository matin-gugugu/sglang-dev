#!/usr/bin/env python3
"""Phase70 read-only source, frozen model, runtime and fresh-plan audit."""
...
from contracts import P69,file_sha,frozen_model,validate_blind_contract,validate_plan  # noqa:E402
...
def run_checks(expected:str,plan_path:Path,raw_dir:Path,audit_output:Path,container_image:str)->dict[str,Any]:
 ...
 model_path=P69/"model/multiflow_high_page_residual.json";model=frozen_model();model_checks={"file_sha":file_sha(model_path)==spec["frozen_correction_contract"]["sha256"],"candidate":model.get("candidate_id")==spec["frozen_correction_contract"]["candidate_id"],"feature_family":model.get("feature_family")==spec["frozen_correction_contract"]["feature_family"],"features":len(model.get("feature_names",[]))==spec["frozen_correction_contract"]["expected_features"],"groups":len(model.get("groups",{}))==spec["frozen_correction_contract"]["expected_groups"],"anchor_r67":model.get("frozen_r67_model_sha256")==spec["frozen_correction_contract"]["anchor_r67_sha256"],"activation":model.get("activation_page_threshold")==spec["frozen_correction_contract"]["activation_page_threshold"],"fresh_blind_not_previously_validated":model.get("blind_status",{}).get("fresh_blind_validated") is False,"phase70_targets_closed":model.get("blind_status",{}).get("phase70_targets_opened") is False,"formula":model.get("inference_formula")==spec["frozen_correction_contract"]["formula"]}
 if not all(model_checks.values()):raise RuntimeError({"frozen_model":model_checks})
 phase69_summary=load_json(P69/"summary.json")
 if phase69_summary.get("status")!="PASS" or phase69_summary.get("next_phase_permitted") is not True or phase69_summary.get("fresh_blind_validated") is not False or phase69_summary.get("phase70_targets_opened") is not False:raise RuntimeError("R69 is not the expected development PASS awaiting Phase70 fresh blind")
