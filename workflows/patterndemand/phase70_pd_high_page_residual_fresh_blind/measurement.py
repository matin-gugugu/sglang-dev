#!/usr/bin/env python3
"""Phase70 raw validation and deterministic frozen-model fresh-blind evaluation."""
from __future__ import annotations
import bisect,json,math,statistics
from collections import defaultdict
from pathlib import Path
from typing import Any
from contracts import contract,file_sha,frozen_model,frozen_r67_model,iteration_counts,layout_by_id,load_json,measurement_by_id,payload_vectors,r61_baseline,r65_baseline,validate_plan
...
def _predict_phase67(model_id:str,configuration:str,m:float,b:float,s:float,pages:list[int])->float:
 model=frozen_r67_model();group=model["groups"][f"{model_id}|{configuration}"];pmax=float(max(pages));prest=float(sum(pages)-max(pages));features=[1.0,m,max(0.0,b-m),max(0.0,s-b),pmax,prest,math.sqrt(pmax),math.sqrt(prest)];value=math.fsum(float(a)*float(x) for a,x in zip(group["coefficients"],features));return max(float(model["prediction_floor_us"]),value)
def _predict_phase69(model_id:str,configuration:str,phase67:float,pages:list[int])->float:
 model=frozen_model();group=model["groups"][f"{model_id}|{configuration}"]
 if configuration=="P2D2_MATCHING":features=[0.0,0.0]
 else:
  pmax=float(max(pages));mean_other=float(sum(pages)-max(pages))/max(1,len(pages)-1);features=[max(0.0,pmax-32.0),max(0.0,mean_other-32.0)]
 residual=math.fsum(float(a)*float(x) for a,x in zip(group["coefficients"],features));return max(float(model["prediction_floor_us"]),phase67+residual)
def _predict_r65(model_id:str,configuration:str,m:float,b:float,s:float)->float:
 model=r65_baseline();coefficients=model["groups"][f"{model_id}|{configuration}"];value=float(coefficients["intercept_us"])+float(coefficients["beta_M"])*m+float(coefficients["beta_busy"])*max(0.0,b-m)+float(coefficients["beta_nonbusy"])*max(0.0,s-b);return max(float(model["prediction_floor_us"]),value)
...
def _metric(rows:list[dict],kind:str,value:str)->dict:
 actual=math.fsum(row["actual_concurrent_wave_us"] for row in rows);max_edge=math.fsum(row["max_edge_baseline_us"] for row in rows);r61=math.fsum(row["r61_graph_prediction_us"] for row in rows);r65=math.fsum(row["phase65_prediction_us"] for row in rows);phase67=math.fsum(row["phase67_prediction_us"] for row in rows);phase69=math.fsum(row["phase69_prediction_us"] for row in rows)
 return {"slice_type":kind,"slice_value":value,"points":len(rows),"max_edge_wape":math.fsum(abs(row["max_edge_baseline_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"r61_graph_wape":math.fsum(abs(row["r61_graph_prediction_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"phase65_wape":math.fsum(abs(row["phase65_prediction_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"phase67_wape":math.fsum(abs(row["phase67_prediction_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"phase69_wape":math.fsum(abs(row["phase69_prediction_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"max_edge_signed_bias":(max_edge-actual)/actual,"r61_graph_signed_bias":(r61-actual)/actual,"phase65_signed_bias":(r65-actual)/actual,"phase67_signed_bias":(phase67-actual)/actual,"phase69_signed_bias":(phase69-actual)/actual}
def build_analysis(plan:dict,raw:dict)->dict[str,Any]:
 ...
 metrics=[_metric(rows,kind,value) for (kind,value),rows in sorted(groups.items())];gate=contract()["fresh_blind_acceptance_gate"];overall=next(row for row in metrics if row["slice_type"]=="overall");models=[row for row in metrics if row["slice_type"]=="model"];configurations=[row for row in metrics if row["slice_type"]=="configuration"];model_configurations=[row for row in metrics if row["slice_type"]=="model_configuration"];fine=[row for row in metrics if row["slice_type"]=="configuration_topology"];shared=set(contract()["research_scope"]["shared_endpoint_configurations"])
 checks={"overall_wape":overall["phase69_wape"]<=gate["overall_wape_max"],"each_model_wape":all(row["phase69_wape"]<=gate["each_model_wape_max"] for row in models),"each_configuration_wape":all(row["phase69_wape"]<=gate["each_configuration_wape_max"] for row in configurations),"each_model_configuration_wape":all(row["phase69_wape"]<=gate["each_model_configuration_wape_max"] for row in model_configurations),"each_configuration_topology_wape":all(row["phase69_wape"]<=gate["each_configuration_topology_wape_max"] for row in fine),"overall_signed_bias":abs(overall["phase69_signed_bias"])<=gate["overall_absolute_signed_bias_max"],"each_model_signed_bias":all(abs(row["phase69_signed_bias"])<=gate["each_model_absolute_signed_bias_max"] for row in models),"each_configuration_signed_bias":all(abs(row["phase69_signed_bias"])<=gate["each_configuration_absolute_signed_bias_max"] for row in configurations),"each_model_configuration_signed_bias":all(abs(row["phase69_signed_bias"])<=gate["each_model_configuration_absolute_signed_bias_max"] for row in model_configurations),"each_configuration_topology_signed_bias":all(abs(row["phase69_signed_bias"])<=gate["each_configuration_topology_absolute_signed_bias_max"] for row in fine),"all_predictions_positive":all(row["phase69_prediction_us"]>0 for row in points),"strictly_improves_all_four_baselines_overall":all(overall["phase69_wape"]<overall[key] for key in ("max_edge_wape","r61_graph_wape","phase65_wape","phase67_wape")),"strictly_improves_best_baseline_each_shared_endpoint_configuration":all(row["phase69_wape"]<min(row["max_edge_wape"],row["r61_graph_wape"],row["phase65_wape"],row["phase67_wape"]) for row in configurations if row["slice_value"] in shared),"strictly_preserves_r67_for_p2d2_matching":all(_close(row["phase69_prediction_us"],row["phase67_prediction_us"]) for row in points if row["configuration"]=="P2D2_MATCHING")};passed=all(checks.values());outcome="MULTIFLOW_HIGH_PAGE_RESIDUAL_THIRD_FRESH_BLIND_PASS" if passed else "MULTIFLOW_HIGH_PAGE_RESIDUAL_THIRD_FRESH_BLIND_FAIL_RETAIN_AS_BLIND_EVIDENCE"
 return {"points":points,"replica_points":replica_points,"spreads":spreads,"metrics":metrics,"decision":{"scientific_outcome":outcome,"fresh_blind_gate_pass":passed,"checks":checks,"thresholds":gate,"training_performed":False,"recalibration_performed":False,"threshold_tuning_performed":False,"phase70_labels_used_for_fitting":False}}
