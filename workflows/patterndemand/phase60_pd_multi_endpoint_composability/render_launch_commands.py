...
def main()->None:
    ...
    node_count=len({row["host"] for row in commands});print(json.dumps({"schema_version":"phase60-launch-command-set-v1","measurement_id":m["measurement_id"],"model_id":m["model_id"],"configuration":m["configuration"],"topology_level":m["topology_level"],"repeat_id":a.repeat_id,"resource_contract":{"inventory_slots_are_gpu_slots_not_nodes":True,"simultaneous_gpu_processes":3,"simultaneous_nodes":node_count,"maximum_simultaneous_nodes":2,"four_node_allocation_required":False,"unused_inventory_slot_not_launched":True},"world_size":3,"commands_must_start_concurrently":True,"one_process_per_command":True,"shared_output_must_not_exist":str(output),"commands":commands},ensure_ascii=False,indent=2))
