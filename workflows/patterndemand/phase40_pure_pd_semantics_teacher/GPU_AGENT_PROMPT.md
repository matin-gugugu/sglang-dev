...
从控制端明确给出的W40创建独立run分支，完整阅读本目录`README_CN.md`和`experiment.json`。本阶段需要同一台机器两张GPU和一个已经存在于本地的Qwen3-8B类模型；禁止联网下载模型。

严格按README依次执行preflight、run和verify。P与D各只使用一张GPU，必须显式保持`TP=1、PP=1`；Mooncake/RDMA、FCFS、4096-token chunk、关闭cache/overlap等均为实验语义，不是可调参数。允许在任何正式raw产生前诊断端口、HCA、GPU可见性、Mooncake初始化和模型路径，允许换同机GPU对或空闲端口；一旦产生首条正式raw，不得换backend、transport、模型、策略、GPU对、HCA、请求顺序或重复数。
