...
正式preflight和run必须设置`HF_HUB_OFFLINE=1`、`TRANSFORMERS_OFFLINE=1`，并让仓库`python/`成为`PYTHONPATH`第一项。选择一对空闲且不同的B200级GPU和一个ACTIVE IB HCA；五个模型顺序复用这对GPU。先preflight，再一条run命令，再verify。

你可以在`AUTO`范围诊断端口占用、同类GPU/HCA、依赖和模型路径，失败重试必须使用新的raw/smoke目录并保留旧证据。不得换模型/revision/backend/page size/transport，不能改P/D内部TP/PP，不能关闭wave barrier，不能降低场景或重复数，不能删异常模型。KV cache保持BF16语义，命令行固定使用TRTLLM MLA接受的`--kv-cache-dtype bf16`拼写，不得改回`bfloat16`或`auto`。任一科学精确门失败时不得把它包装成PASS。
