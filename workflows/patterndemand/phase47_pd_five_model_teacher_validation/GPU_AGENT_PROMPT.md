...
你可以在`AUTO`范围诊断端口占用、同类GPU/HCA、依赖和模型路径，失败重试必须使用新的raw/smoke目录并保留旧证据。不得换模型/revision/backend/page size/transport，不能改P/D内部TP/PP，不能关闭wave barrier，不能降低场景或重复数，不能删异常模型。任一科学精确门失败时不得把它包装成PASS。
