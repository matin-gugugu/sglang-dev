#!/usr/bin/env python3
"""Freeze target-free, historically disjoint Phase45 blind windows."""
...
SEED = "phase45-pd-fresh-blind-v1-20260817"
HISTORY_MS = 300_000
...
def select(root: Path) -> list[dict[str, object]]:
    ...
    for segment in SEGMENTS:
        ...
        for stratum in range(10):
            ...
            pool["selection_order_sha256"] = [digest(str(value)) for value in pool["window_id"]]
            chosen = []
            for _, row in pool.sort_values("selection_order_sha256", kind="stable").iterrows():
                ...
                if len(chosen) == 10:
                    break
            if len(chosen) != 10:
                raise RuntimeError(f"insufficient disjoint blind windows: {segment}/{stratum}/{len(chosen)}")
            for row in chosen:
                segment_rows.append({
        ...
        if len(segment_rows) != 100:
            raise RuntimeError({"segment": segment, "rows": len(segment_rows)})
        for index, row in enumerate(segment_rows, 1):
            row["profile_id"] = f"phase45_{segment}_fresh_blind_{index:03d}"
        selected.extend(segment_rows)
    selected.sort(key=lambda row: (str(row["segment"]), int(row["cutoff_ms"])))
    if len(selected) != 300 or Counter(row["segment"] for row in selected) != Counter({segment: 100 for segment in SEGMENTS}):
        raise RuntimeError("frozen blind selection total differs")
    return selected
