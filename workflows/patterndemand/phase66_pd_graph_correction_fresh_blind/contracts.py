#!/usr/bin/env python3
"""Phase66 reserved payload, frozen model, fresh placement and plan contracts."""
...
def reserved_grid()->dict[str,Any]:return load_json(P65/"contracts/phase66_reserved_blind_grid.json")
def phase64_graph_grid()->dict[str,Any]:return load_json(P64/"contracts/graph_payload_grid.json")
def frozen_model()->dict[str,Any]:return load_json(P65/"model/multiflow_graph_correction.json")
...
def payload_vectors(model_id:str,configuration:str)->list[dict[str,Any]]:
 layout=layout_by_id(model_id);knots={int(row["page_count"]):row for row in layout["knots"]};value=graph(configuration);output=[]
 for index,pages in enumerate(value["page_vectors"]):
  ...
  output.append({"vector_id":f"{model_id}__{configuration.lower()}__blind_v{index:02d}","pages":[int(page) for page in pages],"flows":flows})
 return output
def validate_blind_contract(spec:dict[str,Any]|None=None)->dict[str,Any]:
 spec=spec or contract();grid=reserved_grid();base={key:value for key,value in grid.items() if key!="grid_sha256"};old=phase64_graph_grid();errors=[]
 expected_configs=spec["research_scope"]["fixed_configurations"]
 if grid.get("schema_version")!="phase66-reserved-multiflow-blind-grid-v1" or grid.get("grid_sha256")!=canonical_sha(base) or grid.get("grid_sha256")!=spec["blind_payload_contract"]["canonical_grid_sha256"]:errors.append("grid_identity")
 if grid.get("frozen_before_phase66_raw") is not True or grid.get("phase66_targets_opened") is not False:errors.append("blind_not_closed")
 if grid.get("models")!=spec["selected_models"] or set(grid.get("configurations",{}))!=set(expected_configs):errors.append("scope")
 old_pages={int(page) for value in old["configurations"].values() for vector in value["page_vectors"] for page in vector};new_pages={int(page) for values in grid.get("configurations",{}).values() for vector in values for page in vector}
 if old_pages!=set(spec["blind_payload_contract"]["phase64_development_pages"]) or new_pages!=set(spec["blind_payload_contract"]["reserved_pages"]) or old_pages&new_pages:errors.append("page_boundary")
 expected_edges={"P1D4":4,"P4D1":4,"P2D2_MATCHING":2,"P2D2_ALL_TO_ALL":4};ids=[]
 for model in spec["selected_models"]:
  valid_pages={int(knot["page_count"]) for knot in layout_by_id(model)["knots"]}
  for configuration,count in expected_edges.items():
   ...
   if any(len(row["pages"])!=count or not set(row["pages"])<=valid_pages for row in vectors):errors.append(f"payload:{model}:{configuration}")
 model_value=frozen_model()
 if file_sha(P65/"model/multiflow_graph_correction.json")!=spec["frozen_correction_contract"]["sha256"] or model_value.get("candidate_id")!=spec["frozen_correction_contract"]["candidate_id"] or len(model_value.get("groups",{}))!=spec["frozen_correction_contract"]["expected_groups"]:errors.append("frozen_model")
 if len(ids)!=80 or len(set(ids))!=80:errors.append("vector_cardinality")
 if errors:raise RuntimeError({"invalid_phase66_blind_contract":errors})
 return {"ok":True,"grid_sha256":grid["grid_sha256"],"vectors":80,"reserved_pages":sorted(new_pages),"development_pages":sorted(old_pages),"configuration_flow_counts":expected_edges,"frozen_model_sha256":file_sha(P65/"model/multiflow_graph_correction.json")}
...
def _phase64_plan()->dict[str,Any]:return load_json(P64/"contracts/topology_plan.json")
def phase64_endpoint_keys()->set[tuple[str,int,str]]:return {_endpoint_key(endpoint) for measurement in _phase64_plan()["measurements"] for endpoint in measurement["ranks"]}
def phase64_host_signatures()->dict[str,set[tuple[str,...]]]:
 output:dict[str,set[tuple[str,...]]]=defaultdict(set)
 for measurement in _phase64_plan()["measurements"]:output[measurement["topology_level"]].add(tuple(sorted({row["host"] for row in measurement["ranks"]})))
 return output
def validate_inventory(inventory:dict[str,Any],spec:dict[str,Any]|None=None)->dict[str,Any]:
 spec=spec or contract();errors=[]
 if inventory.get("schema_version")!="phase66-topology-inventory-v1" or inventory.get("blind_inventory_frozen_before_phase66_raw") is not True or inventory.get("selection_uses_phase66_latency_prediction_or_error") is not False:errors.append("inventory_header")
 expected_freshness={"all_endpoint_tuples_absent_from_phase64":True,"minimum_new_host_signatures_per_topology":int(spec["fresh_placement_contract"]["minimum_new_host_signatures_per_topology"]),"phase64_plan_sha256":file_sha(P64/"contracts/topology_plan.json")}
 if inventory.get("freshness_contract")!=expected_freshness:errors.append("freshness_contract")
 peak=inventory.get("phase66_peak_allocation_contract",{});expected_peak={"preferred_scheduler_reserved_nodes":4,"maximum_scheduler_reserved_nodes":4,"maximum_active_measurement_nodes_per_shard":2,"global_peak_simultaneous_gpu_processes":5,"maximum_concurrent_measurement_shards":1,"four_node_scheduler_reservation_permitted":True,"two_measurement_shards_concurrent_forbidden":True,"inventory_slots_are_not_nodes":True}
 ...
 expected_keys={(level,replica) for level in ("L1","L2","L3") for replica in (0,1)};normalized=[];seen=set();endpoint_signatures={};new_host_counts=Counter();old_endpoints=phase64_endpoint_keys();old_hosts=phase64_host_signatures()
 for row in inventory.get("placements",[]):
  try:
   ...
   if overlap:raise ValueError(f"Phase64 endpoint tuple reuse: {overlap}")
 ...
 if errors:raise RuntimeError({"invalid_phase66_inventory":errors})
 return {"ok":True,"placements":6,"endpoint_slots":48,"scheduler_reservation_mode":reservation_mode,"inventory_unique_hosts":len(unique_hosts),"maximum_scheduler_reserved_nodes":4,"maximum_active_measurement_nodes":2,"maximum_simultaneous_gpu_processes":5,"phase64_endpoint_overlap_count":0,"new_host_signatures_by_topology":dict(new_host_counts),"normalized_placements":sorted(normalized,key=lambda row:(row["topology_level"],row["replica_id"]))}
...
def expand_plan(inventory:dict[str,Any],inventory_sha256:str,generated_at_utc:str,workflow_commit:str,spec:dict[str,Any]|None=None)->dict[str,Any]:
 ...
 base={"schema_version":"phase66-topology-plan-v1","workflow_commit":workflow_commit,"generated_at_utc":generated_at_utc,"inventory_file_sha256":inventory_sha256,"reserved_grid_sha256":blind_audit["grid_sha256"],"frozen_model_sha256":blind_audit["frozen_model_sha256"],"placement_summary":{key:value for key,value in inventory_audit.items() if key not in ("ok","normalized_placements")},"resource_contract":spec["resource_allocation_contract"],"measurements":measurements};return {**base,"plan_sha256":canonical_sha(base)}
def validate_plan(plan:dict[str,Any])->dict[str,Any]:
 errors=[]
 if plan.get("schema_version")!="phase66-topology-plan-v1" or not isinstance(plan.get("workflow_commit"),str) or len(plan["workflow_commit"])!=40:errors.append("header")
 ...
 if plan.get("reserved_grid_sha256")!=blind["grid_sha256"] or plan.get("frozen_model_sha256")!=blind["frozen_model_sha256"]:errors.append("blind_binding")
 ...
 if plan.get("placement_summary",{}).get("phase64_endpoint_overlap_count")!=0 or any(plan.get("placement_summary",{}).get("new_host_signatures_by_topology",{}).get(level,0)<1 for level in ("L1","L2","L3")):errors.append("fresh_placement_summary")
 if errors:raise RuntimeError({"invalid_phase66_plan":errors})
 return {"ok":True,"plan_sha256":plan["plan_sha256"],"measurements":48,"endpoint_slots":48,"maximum_world_size":5,"phase64_endpoint_overlap_count":0}
