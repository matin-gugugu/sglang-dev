...
from contracts import P64,contract,expand_plan,iteration_counts,layout_by_id,payload_vectors,phase64_endpoint_keys,validate_blind_contract,validate_inventory,validate_plan
from measurement import _curve_map,_graph_features,_interpolate,_predict_phase65,build_analysis,validate_raw
...
def inventory()->dict:
 ...
 return {"schema_version":"phase66-topology-inventory-v1","blind_inventory_frozen_before_phase66_raw":True,"selection_uses_phase66_latency_prediction_or_error":False,"scheduler_reservation_mode":"FOUR_NODE_SINGLE_ALLOCATION","freshness_contract":{"all_endpoint_tuples_absent_from_phase64":True,"minimum_new_host_signatures_per_topology":1,"phase64_plan_sha256":"5c05154e13866deb0a30cbad32895f6c1db3936ea6359961e3297f4bbfac1819"},"phase66_peak_allocation_contract":{"preferred_scheduler_reserved_nodes":4,"maximum_scheduler_reserved_nodes":4,"maximum_active_measurement_nodes_per_shard":2,"global_peak_simultaneous_gpu_processes":5,"maximum_concurrent_measurement_shards":1,"four_node_scheduler_reservation_permitted":True,"two_measurement_shards_concurrent_forbidden":True,"inventory_slots_are_not_nodes":True},"placements":placements}
...
class Phase66ContractsTest(unittest.TestCase):
 def test_reserved_payload_and_frozen_model(self)->None:
  audit=validate_blind_contract();self.assertEqual(audit["vectors"],80);self.assertEqual(audit["reserved_pages"],[3,6,12,24,32]);self.assertTrue(set(audit["reserved_pages"]).isdisjoint(audit["development_pages"]));self.assertEqual(payload_vectors("deepseek-v2-lite","P2D2_MATCHING")[0]["pages"],[3,3])
 def test_fresh_inventory_and_plan(self)->None:
  audit=validate_inventory(inventory());self.assertEqual(audit["phase64_endpoint_overlap_count"],0);self.assertEqual(audit["inventory_unique_hosts"],4);plan=expand_plan(inventory(),"a"*64,"2026-08-26T00:00:00+00:00","b"*40);plan_audit=validate_plan(plan);self.assertEqual(plan_audit["measurements"],48);self.assertEqual(plan_audit["maximum_world_size"],5);self.assertTrue(all(len({endpoint["host"] for endpoint in measurement["ranks"]})<=2 for measurement in plan["measurements"]))
 def test_phase64_endpoint_reuse_is_rejected(self)->None:
  value=inventory();old=next(iter(phase64_endpoint_keys()));target=value["placements"][0]["sides"]["A"][0];target.update({"host":old[0],"host_aliases":[old[0]],"physical_gpu":old[1],"ib_device":old[2],"transfer_hostname":old[0]})
  with self.assertRaises(RuntimeError):validate_inventory(value)
 def test_frozen_prediction_is_positive(self)->None:
  vector=payload_vectors("qwen3-8b","P1D4")[0];curve=_curve_map()[("qwen3-8b","L1")];costs=[_interpolate(curve,flow["payload_bytes"]) for flow in vector["flows"]];m,b,s=_graph_features(costs,vector["flows"]);prediction=_predict_phase65("qwen3-8b","P1D4",m,b,s);self.assertGreater(prediction,0);self.assertAlmostEqual(prediction,120.861529387723,places=9)
