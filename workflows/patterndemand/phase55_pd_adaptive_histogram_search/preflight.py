#!/usr/bin/env python3
"""Read-only Phase55 preflight; no raw, GPU, network or blind access."""
...
_SPEC = importlib.util.spec_from_file_location("phase54_contracts_for_phase55", P54 / "contracts.py")
...
def run_checks(expected: str) -> dict:
    ...
    head = require_expected_head(expected); require_clean_before_run(allowed_untracked_prefixes=("data/", "experiment-results/phase54_pd_histogram_accuracy_refinement/"))
    source = repo_root() / contract["pinned_inputs"][2]["path"]
    rows = read_csv_gz(source); source_schema = _P54.validate_rows(rows)
    phase48_audit = verify_manifest(repo_root() / contract["dataset_contract"]["source_result_dir"])
    search = contract["search_contract"]
    checks = {
        "seed_candidates_10": int(search["stage_a_seed_candidates"]) == 10,
        "top_k_3": int(search["stage_a_top_k"]) == 3,
        "max_total_candidates_16": int(search["max_total_candidates"]) == 16,
