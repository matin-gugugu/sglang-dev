#!/usr/bin/env python3
"""Verify Phase55 search trace, result tree and development gates."""
...
def verify(output: Path) -> dict:
    ...
    checks = {
        ...
        "candidate_budget_16": len(candidates) == 16 and len(trace) == 16 and summary["counts"]["candidates"] == 16,
        "stage_counts": search["stage_a_count"] == 10 and search["stage_b_count"] == 6,
        "predictions_2880": len(predictions) == 2880,
        "prediction_methods": {row["method"] for row in predictions} == {"h0", "h0_plus_dnn_adaptive"},
        "profiles_240": len({row["profile_id"] for row in predictions}) == 240,
        "models_6": len({row["model"] for row in predictions}) == 6,
        "counts_exact": summary["counts"]["profiles"] == 1200 and summary["counts"]["train_profiles"] == 960 and summary["counts"]["validation_profiles"] == 240 and summary["counts"]["models"] == 6 and summary["counts"]["example_rows"] == 7200 and summary["counts"]["train_rows"] == 5760 and summary["counts"]["validation_rows"] == 1440 and summary["counts"]["complete_request_rows_in_git"] == 0,
        ...
        "no_phase50_blind": checkpoint["phase50_blind_accessed"] is False and search["phase50_blind_accessed"] is False and gates["phase56_permitted"] == gates["target_met"],
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--output-dir", type=Path, default=repo_root() / "experiment-results/phase55_pd_adaptive_histogram_search")
