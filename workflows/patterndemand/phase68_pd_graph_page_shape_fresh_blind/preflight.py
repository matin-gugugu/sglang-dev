#!/usr/bin/env python3
"""Phase68 read-only source, frozen model, runtime and fresh-plan audit."""
...
from contracts import P67,file_sha,frozen_model,validate_blind_contract,validate_plan  # noqa:E402
...
def run_checks(expected:str,plan_path:Path,raw_dir:Path,audit_output:Path,container_image:str)->dict[str,Any]:
 ...
 model_path=P67/"model/multiflow_graph_page_correction.json";model=frozen_model();model_checks={"file_sha":file_sha(model_path)==spec["frozen_correction_contract"]["sha256"],"candidate":model.get("candidate_id")==spec["frozen_correction_contract"]["candidate_id"],"feature_family":model.get("feature_family")==spec["frozen_correction_contract"]["feature_family"],"features":len(model.get("feature_names",[]))==spec["frozen_correction_contract"]["expected_features"],"groups":len(model.get("groups",{}))==spec["frozen_correction_contract"]["expected_groups"],"fresh_blind_not_previously_validated":model.get("blind_status",{}).get("fresh_blind_validated") is False,"phase68_targets_closed":model.get("blind_status",{}).get("phase68_targets_opened") is False,"formula":model.get("inference_formula")==spec["frozen_correction_contract"]["formula"]}
 if not all(model_checks.values()):raise RuntimeError({"frozen_model":model_checks})
 phase67_summary=load_json(P67/"summary.json")
 if phase67_summary.get("status")!="PASS" or phase67_summary.get("next_phase_permitted") is not True or phase67_summary.get("fresh_blind_validated") is not False or phase67_summary.get("phase68_targets_opened") is not False:raise RuntimeError("R67 is not the expected development PASS awaiting Phase68 fresh blind")
