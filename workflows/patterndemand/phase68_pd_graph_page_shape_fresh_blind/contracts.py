#!/usr/bin/env python3
"""Phase68 reserved payload, frozen model, fresh placement and plan contracts."""
...
def reserved_grid()->dict[str,Any]:return load_json(P67/"contracts/phase68_reserved_blind_grid.json")
def phase64_graph_grid()->dict[str,Any]:return load_json(P64/"contracts/graph_payload_grid.json")
def frozen_model()->dict[str,Any]:return load_json(P67/"model/multiflow_graph_page_correction.json")
...
def payload_vectors(model_id:str,configuration:str)->list[dict[str,Any]]:
 ...
 for index,pages in enumerate(value["page_vectors"]):
  ...
  output.append({"vector_id":f"{model_id}__{configuration.lower()}__second_blind_v{index:02d}","pages":[int(page) for page in pages],"flows":flows})
 return output
def validate_blind_contract(spec:dict[str,Any]|None=None)->dict[str,Any]:
 spec=spec or contract();grid=reserved_grid();base={key:value for key,value in grid.items() if key!="grid_sha256"};old=phase64_graph_grid();prior66=load_json(P66/"contracts/reserved_blind_grid.json");errors=[]
 expected_configs=spec["research_scope"]["fixed_configurations"]
 if grid.get("schema_version")!="phase68-reserved-multiflow-second-blind-grid-v1" or grid.get("grid_sha256")!=canonical_sha(base) or grid.get("grid_sha256")!=spec["blind_payload_contract"]["canonical_grid_sha256"]:errors.append("grid_identity")
 if grid.get("frozen_before_phase67_fit") is not True or grid.get("phase68_targets_opened") is not False:errors.append("blind_not_closed")
 ...
 if file_sha(P67/"model/multiflow_graph_page_correction.json")!=spec["frozen_correction_contract"]["sha256"] or model_value.get("candidate_id")!=spec["frozen_correction_contract"]["candidate_id"] or model_value.get("feature_family")!=spec["frozen_correction_contract"]["feature_family"] or len(model_value.get("groups",{}))!=spec["frozen_correction_contract"]["expected_groups"] or len(model_value.get("feature_names",[]))!=spec["frozen_correction_contract"]["expected_features"]:errors.append("frozen_model")
 if len(ids)!=80 or len(set(ids))!=80:errors.append("vector_cardinality")
 if errors:raise RuntimeError({"invalid_phase68_blind_contract":errors})
 return {"ok":True,"grid_sha256":grid["grid_sha256"],"vectors":80,"reserved_pages":sorted(new_pages),"development_pages":sorted(prior_pages),"configuration_flow_counts":expected_edges,"frozen_model_sha256":file_sha(P67/"model/multiflow_graph_page_correction.json")}
...
def _prior_plans()->list[dict[str,Any]]:return [load_json(P64/"contracts/topology_plan.json"),load_json(P66/"contracts/topology_plan.json")]
...
def validate_inventory(inventory:dict[str,Any],spec:dict[str,Any]|None=None)->dict[str,Any]:
 ...
 if inventory.get("schema_version")!="phase68-topology-inventory-v1" or inventory.get("blind_inventory_frozen_before_phase68_raw") is not True or inventory.get("selection_uses_phase68_latency_prediction_or_error") is not False:errors.append("inventory_header")
 expected_freshness={"all_endpoint_tuples_absent_from_phase64_and_phase66":True,"minimum_new_host_signatures_per_topology":int(spec["fresh_placement_contract"]["minimum_new_host_signatures_per_topology"]),"phase64_plan_sha256":file_sha(P64/"contracts/topology_plan.json"),"phase66_plan_sha256":file_sha(P66/"contracts/topology_plan.json")}
 if inventory.get("freshness_contract")!=expected_freshness:errors.append("freshness_contract")
 peak=inventory.get("phase68_peak_allocation_contract",{});expected_peak={"preferred_scheduler_reserved_nodes":4,"maximum_scheduler_reserved_nodes":4,"maximum_active_measurement_nodes_per_shard":2,"global_peak_simultaneous_gpu_processes":5,"maximum_concurrent_measurement_shards":1,"four_node_scheduler_reservation_permitted":True,"two_measurement_shards_concurrent_forbidden":True,"inventory_slots_are_not_nodes":True}
 ...
 for row in inventory.get("placements",[]):
  try:
   ...
   if overlap:raise ValueError(f"Phase64/66 endpoint tuple reuse: {overlap}")
 ...
 if errors:raise RuntimeError({"invalid_phase68_inventory":errors})
...
def validate_plan(plan:dict[str,Any])->dict[str,Any]:
 ...
 if errors:raise RuntimeError({"invalid_phase68_plan":errors})
