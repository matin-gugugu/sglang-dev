#!/usr/bin/env python3
"""Phase68 raw validation and deterministic frozen-model fresh-blind evaluation."""
from __future__ import annotations
import bisect,json,math,statistics
from collections import defaultdict
from pathlib import Path
from typing import Any
from contracts import contract,file_sha,frozen_model,iteration_counts,layout_by_id,load_json,measurement_by_id,payload_vectors,r61_baseline,r65_baseline,validate_plan
...
def _metric(rows:list[dict],kind:str,value:str)->dict:
 actual=math.fsum(row["actual_concurrent_wave_us"] for row in rows);max_edge=math.fsum(row["max_edge_baseline_us"] for row in rows);r61=math.fsum(row["r61_graph_prediction_us"] for row in rows);r65=math.fsum(row["phase65_prediction_us"] for row in rows);phase67=math.fsum(row["phase67_prediction_us"] for row in rows)
 return {"slice_type":kind,"slice_value":value,"points":len(rows),"max_edge_wape":math.fsum(abs(row["max_edge_baseline_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"r61_graph_wape":math.fsum(abs(row["r61_graph_prediction_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"phase65_wape":math.fsum(abs(row["phase65_prediction_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"phase67_wape":math.fsum(abs(row["phase67_prediction_us"]-row["actual_concurrent_wave_us"]) for row in rows)/actual,"max_edge_signed_bias":(max_edge-actual)/actual,"r61_graph_signed_bias":(r61-actual)/actual,"phase65_signed_bias":(r65-actual)/actual,"phase67_signed_bias":(phase67-actual)/actual}
def build_analysis(plan:dict,raw:dict)->dict[str,Any]:
 ...
 checks={"overall_wape":overall["phase67_wape"]<=gate["overall_wape_max"],"each_model_wape":all(row["phase67_wape"]<=gate["each_model_wape_max"] for row in models),"each_configuration_wape":all(row["phase67_wape"]<=gate["each_configuration_wape_max"] for row in configurations),"each_configuration_topology_wape":all(row["phase67_wape"]<=gate["each_configuration_topology_wape_max"] for row in fine),"overall_signed_bias":abs(overall["phase67_signed_bias"])<=gate["overall_absolute_signed_bias_max"],"each_model_signed_bias":all(abs(row["phase67_signed_bias"])<=gate["each_model_absolute_signed_bias_max"] for row in models),"each_configuration_signed_bias":all(abs(row["phase67_signed_bias"])<=gate["each_configuration_absolute_signed_bias_max"] for row in configurations),"each_configuration_topology_signed_bias":all(abs(row["phase67_signed_bias"])<=gate["each_configuration_topology_absolute_signed_bias_max"] for row in fine),"all_predictions_positive":all(row["phase67_prediction_us"]>0 for row in points),"strictly_improves_all_three_baselines_overall":all(overall["phase67_wape"]<overall[key] for key in ("max_edge_wape","r61_graph_wape","phase65_wape")),"strictly_improves_best_baseline_each_configuration":all(row["phase67_wape"]<min(row["max_edge_wape"],row["r61_graph_wape"],row["phase65_wape"]) for row in configurations)};passed=all(checks.values());outcome="MULTIFLOW_GRAPH_PAGE_SHAPE_SECOND_FRESH_BLIND_PASS" if passed else "MULTIFLOW_GRAPH_PAGE_SHAPE_SECOND_FRESH_BLIND_FAIL_RETAIN_AS_BLIND_EVIDENCE"
 return {"points":points,"replica_points":replica_points,"spreads":spreads,"metrics":metrics,"decision":{"scientific_outcome":outcome,"fresh_blind_gate_pass":passed,"checks":checks,"thresholds":gate,"training_performed":False,"recalibration_performed":False,"threshold_tuning_performed":False,"phase68_labels_used_for_fitting":False}}
