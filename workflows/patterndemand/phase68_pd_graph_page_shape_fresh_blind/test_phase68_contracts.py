...
from contracts import P64,P66,expand_plan,iteration_counts,layout_by_id,load_json,payload_vectors,prior_endpoint_keys,validate_blind_contract,validate_inventory,validate_plan
from measurement import _curve_map,_graph_features,_interpolate,_predict_phase67,build_analysis,validate_raw
...
def inventory()->dict:
 ...
 return {"schema_version":"phase68-topology-inventory-v1","blind_inventory_frozen_before_phase68_raw":True,"selection_uses_phase68_latency_prediction_or_error":False,"scheduler_reservation_mode":"FOUR_NODE_SINGLE_ALLOCATION","freshness_contract":{"all_endpoint_tuples_absent_from_phase64_and_phase66":True,"minimum_new_host_signatures_per_topology":1,"phase64_plan_sha256":"5c05154e13866deb0a30cbad32895f6c1db3936ea6359961e3297f4bbfac1819","phase66_plan_sha256":"1898e3615761c1d7d6230a9e5ea4943f8cd024d60bb98e6e1d24faed4b5d62ba"},"phase68_peak_allocation_contract":{"preferred_scheduler_reserved_nodes":4,"maximum_scheduler_reserved_nodes":4,"maximum_active_measurement_nodes_per_shard":2,"global_peak_simultaneous_gpu_processes":5,"maximum_concurrent_measurement_shards":1,"four_node_scheduler_reservation_permitted":True,"two_measurement_shards_concurrent_forbidden":True,"inventory_slots_are_not_nodes":True},"placements":placements}
...
class Phase68ContractsTest(unittest.TestCase):
 def test_reserved_payload_and_frozen_model(self)->None:
  audit=validate_blind_contract();self.assertEqual(audit["vectors"],80);self.assertEqual(audit["reserved_pages"],[36,40,48,56,64]);self.assertTrue(set(audit["reserved_pages"]).isdisjoint(audit["development_pages"]));vector=payload_vectors("deepseek-v2-lite","P2D2_MATCHING")[0];self.assertEqual(vector["pages"],[36,36]);self.assertEqual(vector["flows"][0]["payload_bytes"],1990656*36);self.assertEqual(vector["flows"][0]["descriptor_bytes"],73728*36)
 ...
 def test_frozen_prediction_is_positive(self)->None:
  vector=payload_vectors("qwen3-8b","P1D4")[0];curve=_curve_map()[("qwen3-8b","L1")];costs=[_interpolate(curve,flow["payload_bytes"]) for flow in vector["flows"]];m,b,s=_graph_features(costs,vector["flows"]);prediction=_predict_phase67("qwen3-8b","P1D4",m,b,s,vector["pages"]);self.assertGreater(prediction,0);self.assertTrue(math.isfinite(prediction))
