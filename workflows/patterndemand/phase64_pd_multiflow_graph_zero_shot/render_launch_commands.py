...
def main()->None:
 ...
 print(json.dumps({"schema_version":"phase64-launch-command-set-v1","measurement_id":m["measurement_id"],"model_id":m["model_id"],"configuration":m["configuration"],"topology_level":m["topology_level"],"repeat_id":a.repeat_id,"world_size":m["world_size"],"commands_must_start_concurrently":True,"shared_output_must_not_exist":str(output),"resource_contract":{"simultaneous_nodes":nodes,"maximum_simultaneous_nodes":2,"simultaneous_gpu_processes":len(commands),"maximum_simultaneous_gpu_processes":5,"maximum_concurrent_measurement_shards":1,"four_node_allocation_required":False,"four_node_simultaneous_allocation_forbidden":True},"commands":commands},ensure_ascii=False,indent=2))
